// Code generated by *generator.SegmentUnmarshalerGenerator; DO NOT EDIT.

package segment

import (
	"bytes"
	"fmt"

	"github.com/mitch000001/go-hbci/element"
)

var (
	_ BankSegment = &BankAnnouncementSegment{}
)

func init() {
	s := BankAnnouncementSegment{}
	KnownSegments.mustAddToIndex(VersionedSegment{s.ID(), s.Version()}, func() Segment { return &BankAnnouncementSegment{} })
}

func (b *BankAnnouncementSegment) UnmarshalHBCI(value []byte) error {
	elements, err := ExtractElements(value)
	if err != nil {
		return err
	}
	if len(elements) == 0 {
		return fmt.Errorf("malformed marshaled value: no elements extracted")
	}
	seg, err := SegmentFromHeaderBytes(elements[0], b)
	if err != nil {
		return err
	}
	b.Segment = seg
	if len(elements) > 1 && len(elements[1]) > 0 {
		b.Subject = &element.AlphaNumericDataElement{}
		err = b.Subject.UnmarshalHBCI(elements[1])
		if err != nil {
			return fmt.Errorf("error unmarshaling Subject: %w", err)
		}
	}
	if len(elements) > 2 && len(elements[2]) > 0 {
		b.Body = &element.TextDataElement{}
		if len(elements)+1 > 2 {
			err = b.Body.UnmarshalHBCI(bytes.Join(elements[2:], []byte("+")))
		} else {
			err = b.Body.UnmarshalHBCI(elements[2])
		}
		if err != nil {
			return fmt.Errorf("error unmarshaling Body: %w", err)
		}
	}
	return nil
}
