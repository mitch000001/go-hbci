...
func (a *AccountInformationResponseSegment) UnmarshalHBCI(value []byte) error {
	...
	if len(elements) > 2 && len(elements[2]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling AccountKind: %w", err)
		}
	}
	...
	if len(elements) > 7 && len(elements[7]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling OpeningDate: %w", err)
		}
	}
	if len(elements) > 8 && len(elements[8]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling DebitInterest: %w", err)
		}
	}
	if len(elements) > 9 && len(elements[9]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling CreditInterest: %w", err)
		}
	}
	if len(elements) > 10 && len(elements[10]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling OverDebitInterest: %w", err)
		}
	}
	...
	if len(elements) > 12 && len(elements[12]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling ReferenceAccount: %w", err)
		}
	}
	if len(elements) > 13 && len(elements[13]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling AccountStatementShippingType: %w", err)
		}
	}
	if len(elements) > 14 && len(elements[14]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling AccountStatementShippingRotation: %w", err)
		}
	}
	if len(elements) > 15 && len(elements[15]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling AdditionalInformation: %w", err)
		}
	}
	if len(elements) > 16 && len(elements[16]) > 0 {
		...
		if err != nil {
			return fmt.Errorf("error unmarshaling DisposalEligiblePersons: %w", err)
		}
	}
	return nil
}
