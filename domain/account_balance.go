...
// SepaAccountBalances represents a printable version of a slice of
// account balances
...
func (ab SepaAccountBalances) String() string {
	var buf bytes.Buffer
	buf.WriteString("\n")
	buf.WriteString("Product Name\tBIC\tIBAN\t\tBookingDate\tAmount\tLimit")
	buf.WriteString("\n")
	containsEarmarkedBalances := false
	for _, a := range ab {
		buf.WriteString(a.ProductName)
		buf.WriteString("\t")
		buf.WriteString(a.Account.BIC)
		buf.WriteString("\t")
		buf.WriteString(a.Account.IBAN)
		buf.WriteString("\t")
		buf.WriteString("\t")
		buf.WriteString(a.BookedBalance.TransmissionDate.Format("2006-01-02"))
		buf.WriteString("\t")
...
type SepaAccountBalance struct {
	Account            InternationalAccountConnection
	ProductName        string
	Currency           string
	BookedBalance      Balance
	EarmarkedBalance   *Balance
	...
	BookingDate        *time.Time
	DueDate            *time.Time
	SeizableAfterMonth *Amount
}

func (a SepaAccountBalance) String() string {
	var buf bytes.Buffer
	buf.WriteString("\n")
	buf.WriteString("BIC\tIBAN\t\tBookingDate\tAmount")
	buf.WriteString("\n")
	buf.WriteString(a.Account.BIC)
	buf.WriteString("\t")
	buf.WriteString(a.Account.IBAN)
